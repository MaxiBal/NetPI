...
inline int error__(const char* msg)
{
	perror(msg);
	return EXIT_FAILURE;
}
...
inline std::map<std::string, std::string> map_request(const std::string& total_request)
{
...
private:
	...
	headers<std::string, std::string> header_list;
...
public:
	...
	void expect_headers( std::string key, std::string value)
	{
		header_list.set_header(key, value);
	}
	...
	int listen_(callback callback_)
	{
		...
		if (ret == -1) {
			return error__("setsockopt() failed");
		} // ret == -1
